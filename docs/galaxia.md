...
B. 발급(Get) 시 파라미터 차이
\* `order-number` (주문번호):
​     \* cpn: tr_id (거래 ID) 그대로 사용 (예: ga_1234_5678)
​     \* dept: tr_id에서 언더바(`_`)를 제거해서 사용 (예: ga12345678)
\* `faceValue` (액면가):
...
C. 조회(Check) / 취소(Cancel) 시 파라미터 차이
\* `order-number` / `tr_id`: 발급 때와 마찬가지로 백화점 상품권(dept)인 경우 언더바(_)를 제거하고 암호화하거나 전송합니다.
