...
-- │ 그 다음: 애플리케이션 배포                                                 │
-- └────────────────────────────────────────────────────────────────────────────┘
-- ※ 197-16 이 요구하는 스키마 변경은 **이 파일 하나뿐**이다.
--   (SSG 발송건별 귀속 컬럼은 SSG 부분취소를 후속 티켓으로 미루면서 함께 뺐다. 그 컬럼을 쓰는
--    코드도 엔티티도 없으므로 지금 적용할 것이 없다 — docs/followup-ssg-partial-cancel.md 참고.)
--
-- ※ 앱 배포보다 **먼저** 적용해야 한다. 엔티티가 이 두 컬럼을 선언하므로, 컬럼 없이 앱이 뜨면
--   order_delivery 를 읽는 조회가 Unknown column 으로 전부 실패한다.
