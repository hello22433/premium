...
--   (다른 테이블). 다만 둘 다 앱 배포보다 먼저 적용돼야 한다 — 엔티티가 이 컬럼을 선언하므로
...
--   적용 직후에는 신규 행도 전부 NULL 이다. 기록·부분복구·차단 가드는 아직 구현돼 있지 않다.
...
--       지금은 (행사 1개 = 1행) 이라 작지만, 이 컬럼을 쓰는 후속 작업이 이력을 발송건 단위로
--       분해하므로 행 수가 크게 늘어난다. 다시 합치지 말 것.
