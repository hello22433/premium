...
export const DeliveryCreateCouponImage = async (
  ...
  try {
    ...
    // 저장 디렉터리: 기본은 기존 동작 유지(<cwd>/public). COUPON_IMAGE_DIR 설정 시 비공개 디렉터리로 이동
    // → public/ 정적 노출 원천 차단. ⚠️ 전환 시 발신사(Gemtek)가 해당 경로를 읽을 수 있어야 함(공유 마운트 확인).
    // 소비처는 반환 path를 그대로 로컬 첨부/DB 저장에 사용하므로 경로만 바뀌고 흐름 무영향.
