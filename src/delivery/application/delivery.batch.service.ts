...
export class DeliveryBatchService {
  ...
  async deliveryDeliveryTargetDestroy() {
    ...
    const orderDeliveryList = await this.orderDeliveryRepository
      ...
      //   파기일수 입력은 62~180일만 허용하므로, 운영자·고객사가 인식하는 기간과 실제 보관
      //   기간이 어긋난다. 이 괴리는 코드 변경만으로 진행하기로 결정됐고(기존 관행), 개인정보
      //   처리방침·고객사 계약 문구와의 정합은 **확인되지 않은 상태로 남아 있다**.
      //   즉 "검토를 마쳤다"가 아니라 "검토 없이 진행했다"가 현재 상태다. 관련 문의나 감사가
      //   들어오면 이 지점부터 확인할 것.
      //   실보유기간을 줄이려면 이 절에 couponStatus 조건을 되돌리는 것이 아니라(그러면 CS 대응
      //   요구가 다시 깨진다) 파기일수 입력 상한을 유효기간에 맞춰 여는 쪽이 맞다.
      //
