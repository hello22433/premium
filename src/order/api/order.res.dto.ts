...
export class OrderGetDeliveryCompleteReportResDto extends OrderGetDeliveryCompleteReportDetailResDto {
  ...
  @ApiProperty({
    ...
    description:
      ...
      '① 조기파기된 건 → 실제 실행일(early_destroy_request.executedAt). **확정된 실적**이다. ' +
      '② 그 외 → `MAX(발송요청일 + 파기일수, 유효기간 만료일 + 1일)` (만료 당일은 아직 유효하므로 +1일). ' +
      '②는 배치가 정상 동작할 때의 **추정치**다 — 환불 진행중이거나 배치 회차가 유실되면 뒤로 밀리고, ' +
      '주문이 발송완료 상태를 이탈하면 파기되지 않는데도 날짜는 계속 표시된다. ' +
      ...
      '⚠️ 폴백 시 주의: 파기일수(requestToDestroyPersonalInfoDay)가 NULL 인 매핑이 정확히 이 필드가 ' +
      'null 이 되는 경우인데, 그 필드는 0 으로 내려간다. 그 값으로 `발송일 + 0일`을 계산하면 발송 당일이 ' +
      '파기일로 표시되므로(실제로는 그 행이 영구 미파기다) 자체 계산에 쓰지 말 것.',
  })
