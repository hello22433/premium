...
export class OrderGetDeliveryCompleteReportResDto extends OrderGetDeliveryCompleteReportDetailResDto {
  ...
  @ApiProperty({
    ...
    description:
      ...
      '파기확인서의 파기일 칸에 쓸 수 있는 것은 이 경우다. ' +
      '② 아직 파기되지 않은 건 → `MAX(발송요청일 + 파기일수, 유효기간 만료일 + 1일)` 로 계산한 **예정일**이다 ' +
      '(만료 당일은 아직 유효하므로 +1일). 배치가 정상 동작할 때의 하한이라 확정이 아니다 — 환불 진행중이거나 ' +
      '배치 회차가 유실되면 뒤로 밀리고, 주문이 발송완료 상태를 이탈하면 파기되지 않는데도 날짜는 계속 표시된다. ' +
      ...
      '파기일을 특정할 수 없으면 null 이며, 이때는 호출부가 종전 계산으로 폴백한다. null 이 되는 경우는 두 가지다: ' +
      '(가) 파기일수/발송요청일이 결측이라 예정일을 계산할 수 없는 건, ' +
      '(나) **이미 파기됐는데 파기 시각 기록이 없는 건** — 지운 것은 확실하나 언제인지 모르므로 날짜를 지어내지 않는다. ' +
      '⚠️ 폴백 시 주의: 파기일수(requestToDestroyPersonalInfoDay)가 NULL 인 매핑이 (가)에 해당하는데, ' +
      '그 필드는 0 으로 내려간다. 그 값으로 `발송일 + 0일`을 계산하면 발송 당일이 파기일로 표시되므로 ' +
      '(실제로는 그 행이 영구 미파기다) 자체 계산에 쓰지 말 것.',
  })
