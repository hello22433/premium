...
export class OrderController {
  ...
  @ApiOperation({
    summary: '파기확약서 이메일 전송 API',
    description: '파기확약서 PDF를 이메일로 전송합니다. 운영관리자 이상만 사용 가능.',
  })
  @ApiBearerAuth()
  @ApiOkResponse({
    description: '성공적으로 전송한 경우',
  ...
  // =========================================
  @UseGuards(AuthUserSuperAndOperationAdminGuard)
  @Post('/order/destruction-certificate/report/email')
  async sendDestructionCertificateReportEmail(
    @Body() getBody: OrderDestructionCertificateEmailReqDto,
    @User() user: ILoginUserInfo,
    @Req() req: Request,
  ) {
    const ipAddress = req.ip || req.headers['x-forwarded-for']?.toString() || '';
