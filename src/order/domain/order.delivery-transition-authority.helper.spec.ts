...
describe('order delivery transition authority', () => {
  ...
  describe('canTransitionDelivery', () => {
    it('직발송은 운영 관리자만 허용한다', () => {
      ...
      expect(
        canTransitionDelivery({ ...operationAdmin, id: 10, authority: IUserAuthority.CORPORATE_ADMIN }, order),
      ).toBe(false);
      expect(canTransitionDelivery(operationAdmin, order)).toBe(true);
      expect(canTransitionDelivery({ ...operationAdmin, authority: IUserAuthority.CORPORATE_ADMIN }, order)).toBe(
        false,
      );
    });
    ...
    it('대행발송은 배정된 운영 담당자만 허용한다', () => {
      ...
      expect(
        canTransitionDelivery({ ...operationAdmin, id: 20, authority: IUserAuthority.CORPORATE_ADMIN }, order),
      ).toBe(false);
      expect(
        canTransitionDelivery({ ...operationAdmin, id: 30, authority: IUserAuthority.CORPORATE_ADMIN }, order),
      ).toBe(false);
    ...
    it('최고 관리자는 모든 주문을 허용한다', () => {
      ...
      expect(canTransitionDelivery({ ...operationAdmin, id: 99, authority: IUserAuthority.SUPER_ADMIN }, order)).toBe(
        true,
      );
    });
  ...
  describe('shouldExposeSsgBalanceCheck', () => {
    ...
    it('고객사(CORPORATE_ADMIN)에게는 노출 안 함(false) — 민감 재무데이터 방어', () => {
      expect(
        shouldExposeSsgBalanceCheck({ ...operationAdmin, authority: IUserAuthority.CORPORATE_ADMIN }, ssgReviewOrder),
      ).toBe(false);
    });
    ...
    it('비-SSG 주문은 노출 안 함(false)', () => {
      expect(shouldExposeSsgBalanceCheck(operationAdmin, { ...ssgReviewOrder, type: IOrderType.GENERAL })).toBe(false);
    });
