...
export class UserDriveService {
   ...
   *   (assertDownloadable)과 맞춘다.
   *     · 같은 칸 — private 첨부의 소유 판정 `ownerId === senderId` (요청자 권한도 업로더의 현재 권한도 안 봄)
   *     · 다른 칸 — 레거시 `image/`·`file/` 는 다운로드는 **허용**, 여기서는 **대상 제외**.
   *       공개 객체라 다운로드를 막을 이유가 없고, 메타데이터가 없어 HeadObject 는 헛호출이기 때문이다.
   *
   ...
   * ※ 남기는 것 — 내부 id(요청자·발신자·업로더), 마스킹된 key, 그리고 **거절 사유가 된 세그먼트 문자열**.
   *   마지막 것은 클라이언트가 정하는 값이라 아래 정제를 반드시 지난다. 원본 파일명은 남지 않는다
   *   (maskStorageKeyForLog 가 식별자 앞부분만 남긴다).
